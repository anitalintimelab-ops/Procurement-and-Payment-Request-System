...
is_reviewer = (curr_name in REVIEWERS)
is_cfo = (curr_name in CFOS)
...
with st.sidebar.expander("🔐 修改我的密碼"):
    ...
    if st.button("更新密碼"):
        ...
        elif not validate_password(new_pw): st.error("不符合規則：至少一英文+數字4-6位")
        else:
...
def render_html(row):
    ...
    h += f'<div style="display:flex;justify-content:space-between;"><span>承辦人：{row["申請人"]} ({row["提交時間"]})</span><span>專案初審：{rev_info}</span></div>'
    h += f'<div style="display:flex;justify-content:space-between;"><span>財務長複審：{cfo_info}</span><span>財務簽核：_________</span></div></div></div>'
...
# --- 6. 主功能流程 ---
m_opts = ["1. 填寫申請單"]
if is_reviewer or is_cfo: m_opts.append("2. 簽核中心")
...
if menu == "1. 填寫申請單":
    ...
    with st.form("apply_form"):
        ...
        vdr = st.text_input("廠商", value=ed_data["請款廠商"] if ed_data is not None else "")
        acc = st.text_input("帳戶", value=ed_data["匯款帳戶"] if ed_data is not None else "")
        desc = st.text_area("說明 *", value=ed_data["請款說明"] if ed_data is not None else "")
        
        acc_f = st.file_uploader("上傳新存摺 (若不更新則不選)", type=["jpg","png"])
        ims_f = st.file_uploader("上傳新憑證 (若不更新則不選)", type=["jpg","png"], accept_multiple_files=True)
        ...
        if st.form_submit_button("💾 儲存內容"):
            if not (app and pn and pi and amt > 0 and desc):
                ...
                if st.session_state.edit_id:
                    ...
                    new_db.at[idx,"專案執行人"], new_db.at[idx,"專案編號"] = exe, pi
                    new_db.at[idx,"總金額"], new_db.at[idx,"請款說明"] = amt, desc
                    new_db.at[idx,"類型"], new_db.at[idx,"付款方式"] = tp, pay
                    new_db.at[idx,"請款廠商"], new_db.at[idx,"匯款帳戶"] = vdr, acc
                    new_db.at[idx,"狀態"] = "草稿"
                    # 強制寫入目前登入者身分，避免消失
                    ...
                    nr = {"單號":tid,"日期":str(datetime.date.today()),"類型":tp,"申請人":app,"專案執行人":exe,"專案名稱":pn,"專案編號":pi,
                          "請款說明":desc,"總金額":amt,"幣別":"TWD","付款方式":pay,"請款廠商":vdr,"匯款帳戶":acc,"帳戶影像Base64":a_b,
                          "狀態":"草稿","影像Base64":i_b,"提交時間":"","申請人信箱":curr_name,
                          "初審人":"","初審時間":"","複審人":"","複審時間":""}
    ...
    if disp_db.empty: 
        ...
        for i, r in disp_db.iterrows():
            ...
            # 狀態顏色與文字修正
            stt = r["狀態"]
            color = "green" if stt == "已核准" else "blue" if stt == "待複審" else "orange" if stt == "待初審" else "red"
            ...
            if cols[5].button("修改", key=f"e_{rid}", disabled=not can_edit):
                ...
                st.rerun()
            if cols[6].button("刪除", key=f"d_{rid}", disabled=not can_edit):
                st.session_state.db = st.session_state.db[st.session_state.db["單號"]!=rid]
                save_data(st.session_state.db)
                st.rerun()
            if cols[7].button("預覽", key=f"v_{rid}"):
                st.session_state.view_id = rid
                st.rerun()
    ...
    if p_df.empty: 
        ...
        for i, r in p_df.iterrows():
            rid = r["單號"]
            with st.expander(f"待審：{rid} - {r['專案名稱']} (申請人：{r['申請人']})"):
                st.markdown(render_html(r), unsafe_allow_html=True)
                c1, c2 = st.columns(2)
                
                if is_reviewer and r["狀態"] == "待初審":
                    if c1.button("✅ 初審核准", key=f"ok_rev_{rid}"):
                    ...
                    if c2.button("❌ 初審駁回", key=f"no_rev_{rid}"):
                        idx = st.session_state.db[st.session_state.db["單號"]==rid].index[0]
                        st.session_state.db.at[idx, "狀態"] = "已駁回"; save_data(st.session_state.db); st.rerun()
                
                if is_cfo and r["狀態"] == "待複審":
                    if c1.button("👑 最終核准", key=f"ok_cfo_{rid}"):
                        idx = st.session_state.db[st.session_state.db["單號"]==rid].index[0]
                        ...
                        save_data(st.session_state.db); st.rerun()
                    if c2.button("❌ 複審駁回", key=f"no_cfo_{rid}"):
                        idx = st.session_state.db[st.session_state.db["單號"]==rid].index[0]
                        st.session_state.db.at[idx, "狀態"] = "已駁回"; save_data(st.session_state.db); st.rerun()
