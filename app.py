...
# [修正] 透過 key 綁定選單，解決修改後無法跳轉頁面的問題
menu = st.sidebar.radio("導覽", menu_options, key="menu_radio")

# [修正預覽 Bug] 只有在「切換左側選單」時，才清除預覽畫面。這樣在同一個頁面按預覽就不會消失了！
...
# --- 頁面 2: 執行長簽核 ---
    ...
    if h_df.empty: st.info("尚無紀錄")
        ...
        # [指令2 & 3] 執行長歷史紀錄加入預覽、列印，以及待複審階段的修改權限
        ...
        for i, r in h_df.iterrows():
            ...
            with lc6:
                ...
                # [指令3] 執行長覆核後在財務長未簽核前(待複審) 仍可以修改
                can_ceo_edit = (r["狀態"] == "待複審") and (r["專案負責人"] == curr_name) and is_active
                if lb3.button("✏️ 修改", key=f"h_ceo_e_{i}", disabled=not can_ceo_edit):
...
# --- 頁面 5: 請款狀態 (Anita 專屬) ---
    ...
    # [指令4] 提供管理員完美備份與還原工具，徹底解決 Streamlit Cloud 隔天重置問題
