...
S_FILE = os.path.join(B_DIR, "staff_v3.csv")
...
def load_staff():
    ...
    # 預設名單
    d = {"name": MANAGERS, "status": ["在職"] * len(MANAGERS)}
...
# --- 3. 側邊欄與管理工具 ---
st.sidebar.markdown(f"### 👤 登入中：{curr_name}")
...
if is_admin:
    ...
    with st.sidebar.expander("💾 資料備份與還原"):
        ...
        st.divider()
        st.write("2. 從電腦還原資料")
        up_file = st.file_uploader("上傳備份檔", type=["csv"])
...
if menu == "1. 填寫申請單":
    ...
    with st.form("apply_form"):
        ...
        with c1:
            ...
            exe = st.selectbox("專案執行人 *", s_list, index=s_list.index(ed_data["專案執行人"]) if (ed_data is not None and ed_data["專案執行人"] in s_list) else 0)
        with c2:
        ...
        st.divider(); acc_f = st.file_uploader("📷 上傳存摺", type=["jpg","png"])
        ims_f = st.file_uploader("📷 上傳憑證", type=["jpg","png"], accept_multiple_files=True)
    ...
    if st.session_state.last_id:
        ...
        if py.button("🚀 提交送審", key="s_gui"):
            ...
            st.session_state.db.at[idx_s, "狀態"] = "待簽核"; st.session_state.db.at[idx_s, "提交時間"] = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
            save_data(st.session_state.db); st.success("✅ 已提交送審！"); st.session_state.last_id = None; st.rerun()
