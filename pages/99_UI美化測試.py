...
# --- 1. 系統鎖定與介面設定 ---
st.session_state['sys_choice'] = "體驗會測試系統" # 區隔正式系統的顯示名稱
...
if is_admin:
    st.sidebar.markdown("---")
    st.sidebar.success("管理員專屬區塊 (展示版)")
            ...
            with st.expander("💾 1. 表單資料庫備份與還原", expanded=True):
                ...
                with col_down:
                    ...
                    if os.path.exists(D_FILE):
                        with open(D_FILE, "rb") as f: st.download_button("下載表單備份檔", f, file_name=f"體驗版表單備份_{datetime.date.today()}.csv", mime="text/csv")
                with col_up:
            ...
            with st.expander("👥 2. 人員與大頭貼資料備份與還原"):
                ...
                with col_down2:
                    ...
                    if os.path.exists(S_FILE):
                        with open(S_FILE, "rb") as f: st.download_button("下載人員備份檔", f, file_name=f"體驗版人員備份_{datetime.date.today()}.csv", mime="text/csv")
                with col_up2:
