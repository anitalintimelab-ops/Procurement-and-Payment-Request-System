...
def render_upload_popover(container, r, prefix):
    with container.popover("📎 附件"):
        ...
        cam_acc = st.camera_input("📸 拍照上傳存摺", key=f"{prefix}_cam_a")
        ...
        # 手機版相機功能 (隱藏在電腦版)
        st.markdown("<div class='mobile-camera-only'>", unsafe_allow_html=True)
        cam_ims = st.camera_input("📸 拍照上傳憑證", key=f"{prefix}_cam_i")
        st.markdown("</div>", unsafe_allow_html=True)
        ...
        if st.button("💾 儲存附件", key=f"{prefix}_b"):
            ...
            # 處理存摺上傳 (優先取相機拍照)
            acc_file = cam_acc if cam_acc else nf_acc
            if acc_file: 
                fresh_db.at[idx, "帳戶影像Base64"] = base64.b64encode(acc_file.getvalue()).decode()
                jd["acc_name"] = "camera_acc.jpg" if acc_file == cam_acc else acc_file.name
...
# 如果沒有進入簽核視窗，則顯示正常選單頁面
    ...
    if menu == "1. 填寫申請單":
        ...
        with st.container():
            ...
            st.markdown("<div class='mobile-camera-only'>", unsafe_allow_html=True)
            cam_acc = st.camera_input("📸 拍照上傳存摺", key=f"req_cam_acc_{up_key}")
            st.markdown("</div>", unsafe_allow_html=True)
            
            f_ims = st.file_uploader("上傳請款憑證 (圖/Excel)", type=["png", "jpg", "xlsx", "xls"], accept_multiple_files=True, key=f"req_f_ims_{up_key}")
            
            st.markdown("<div class='mobile-camera-only'>", unsafe_allow_html=True)
            cam_ims = st.camera_input("📸 拍照上傳憑證", key=f"req_cam_ims_{up_key}")
            st.markdown("</div>", unsafe_allow_html=True)
