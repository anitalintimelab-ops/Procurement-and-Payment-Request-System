...
def render_signing_table(df_list, sign_type, is_history=False):
    ...
    time_header = "最後審核通過日期時間" if is_history else "提交時間"
    headers = ["單號", "專案名稱", "負責執行長", "申請人", "請款金額", time_header, "操作"]
    ...
    for i, r in df_list.iterrows():
        ...
        fallback_date = get_fallback_date(r)
        stt = safe_str(r.get("狀態")).strip()
        t_disp = ""
        
        # 依照歷史紀錄與審核身分，精準顯示對應的「最後審核時間」
        if is_history:
            if sign_type == "EXE":
                t_disp = safe_str(r.get("初審時間"))[:16].strip()
                if not t_disp and stt in ["待複審", "已核准"]:
                    t_disp = f"{fallback_date} 13:00"
                elif t_disp and len(t_disp) <= 10:
                    t_disp = f"{t_disp} 13:00"
            else:
                t_disp = safe_str(r.get("複審時間"))[:16].strip()
                if not t_disp and stt == "已核准":
                    t_disp = f"{fallback_date} 14:00"
                elif t_disp and len(t_disp) <= 10:
                    t_disp = f"{t_disp} 14:00"
        else:
            t_disp = safe_str(r.get("提交時間"))[:16].strip()
            if not t_disp and stt in ["待簽核", "待初審", "待複審", "已核准", "已駁回"]:
                t_disp = f"{fallback_date} 12:00"
            elif t_disp and len(t_disp) <= 10:
                t_disp = f"{t_disp} 12:00"
...
if st.session_state.view_id:
    ...
    if r_df.empty:
        ...
        # ★ 放寬附件抓取條件，兼容多種分隔符號，不再因為長度限制擋掉正常的檔案
