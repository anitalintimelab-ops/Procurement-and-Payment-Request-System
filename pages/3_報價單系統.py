...
def clean_amount(val):
    if pd.isna(val) or val is None or str(val).strip() == "": 
        return 0
    try: 
        return int(float(str(val).replace(",", "").replace("$", "").replace("，", "").replace(" ", "")))
    except Exception: 
        return 0
...
# --- 報價單專屬資料解析器 ---
def parse_quote_data(desc_raw):
    c_name, c_phone, tax_id, address, note = "", "", "", "", desc_raw
    if "[報價單資料]" in desc_raw:
        try:
            json_str = desc_raw.split("[報價單資料]")[1].strip()
            ...
            pass
    return c_name, c_phone, tax_id, address, note
...
with st.sidebar.expander("📸 修改大頭貼"):
    ...
    if st.button("更新大頭貼", disabled=not is_active) and new_avatar:
        ...
        staff_df.at[idx, "avatar"] = base64.b64encode(new_avatar.getvalue()).decode()
        save_staff(staff_df)
...
def render_html(row):
    ...
    # 提取報價單專屬資料
    c_name, c_phone, tax_id, address, note = parse_quote_data(row.get("請款說明", ""))
    ...
    h += f'<div style="text-align:center; border-bottom:2px solid #000; padding-bottom:10px; margin-bottom:10px;"><div style="display:flex; justify-content:center; align-items:center; gap:15px; flex-wrap:wrap;">{lg_html}<h2 style="margin:0;">時研國際設計股份有限公司</h2></div><h3 style="margin:10px 0 0 0; letter-spacing:5px;">工程報價單</h3></div>'
    ...
    h += f'<tr><td bgcolor="#eee">專案名稱</td><td>{row["專案名稱"]}</td><td bgcolor="#eee">專案編號</td><td>{row["專案編號"]}</td></tr>'
    h += f'<tr><td bgcolor="#eee">客戶名稱</td><td>{c_name}</td><td bgcolor="#eee">客戶電話</td><td>{c_phone}</td></tr>'
    h += f'<tr><td bgcolor="#eee">統一編號</td><td>{tax_id}</td><td bgcolor="#eee">施工地址</td><td>{address}</td></tr>'
    h += f'<tr><td bgcolor="#eee">申請人</td><td colspan="3">{display_app}</td></tr>'
    h += f'<tr><td bgcolor="#eee">備註說明</td><td colspan="3">{note}</td></tr>'
    
    c_cur = str(row.get("幣別", "TWD")).replace("nan", "TWD")
    h += f'<tr><td colspan="3" align="right">報價總金額</td><td align="right">{c_cur} {amt:,.0f}</td></tr></table>'
...
def render_upload_popover(container, r, prefix):
    with container.popover("📎 附件"):
        st.write("**上傳新附件 (覆蓋原檔)**")
        new_f_ims = st.file_uploader("報價單附件", type=["png", "jpg", "jpeg", "pdf"], accept_multiple_files=True, key=f"{prefix}_i")
...
if menu == "1. 填寫申請單":
    ...
    try:
        ...
        if curr_name not in staffs: 
            staffs.append(curr_name)
        
        sys_save_type = "報價單" 
        curr_options = ["TWD", "USD", "EUR", "JPY", "CNY", "HKD", "GBP", "AUD"]
        dv = {"pn":"", "exe":staffs[0], "pi":"", "amt":0, "curr":"TWD", "ib64":"", "app": curr_name, "c_name": "", "c_phone": "", "tax_id": "", "address": "", "desc": ""}
        ...
        if st.session_state.edit_id:
            ...
            if not r.empty:
                ...
                c_name, c_phone, tax_id, address, note = parse_quote_data(row.get("請款說明", ""))
                dv.update({
                    "app": clean_name(row.get("申請人", curr_name)), 
                    "pn": str(row.get("專案名稱", "")), 
                    "exe": clean_name(row.get("專案負責人", staffs[0])), 
                    "pi": str(row.get("專案編號", "")), 
                    "amt": clean_amount(row.get("總金額", 0)), 
                    "curr": str(row.get("幣別", "TWD")), 
                    "ib64": str(row.get("影像Base64", "")),
                    "c_name": c_name,
                    "c_phone": c_phone,
                    "tax_id": tax_id,
                    "address": address,
        ...
        with st.form("form"):
            ...
            st.markdown("---")
            st.markdown("**(報價單專屬客戶資料)**")
            cp1, cp2 = st.columns(2)
            c_name = cp1.text_input("客戶名稱", value=dv["c_name"], key=f"c_name_{mode_suffix}")
            c_phone = cp2.text_input("客戶電話", value=dv["c_phone"], key=f"c_phone_{mode_suffix}")
            tax_id = cp1.text_input("統一編號", value=dv["tax_id"], key=f"tax_id_{mode_suffix}")
            address = cp2.text_input("施工地址", value=dv["address"], key=f"address_{mode_suffix}")
            st.markdown("---")
            
            amt = st.number_input("報價總金額", value=int(max(0, dv["amt"])), min_value=0, key=f"amt_{mode_suffix}")
            currency = st.selectbox("幣別", curr_options, index=curr_options.index(dv["curr"]) if dv["curr"] in curr_options else 0, key=f"curr_{mode_suffix}")
            
            desc = st.text_area("備註說明", value=dv["desc"], key=f"desc_{mode_suffix}")
            
            del_ims = False
            if dv["ib64"]:
                st.write("✅ 已有報價單檔案")
                del_ims = st.checkbox("❌ 刪除所有檔案", key=f"di_{mode_suffix}")
            f_ims = st.file_uploader("上傳報價單檔案 (支援圖片與PDF)", type=["png", "jpg", "jpeg", "pdf"], accept_multiple_files=True, key=f"fi_{mode_suffix}")
            
            if st.form_submit_button("💾 儲存", disabled=not is_active):
                db = load_data()
                if not (pn and pi and amt>0):
                    st.error("請確認必填欄位 (專案名稱、編號、金額) 已填寫")
                else:
                    ...
                    # 將客戶資訊打包進請款說明
                    meta_data = {"c_name": c_name, "c_phone": c_phone, "tax_id": tax_id, "address": address, "desc": desc}
                    packed_desc = "[報價單資料]\n" + json.dumps(meta_data, ensure_ascii=False)
                    
                    if st.session_state.edit_id:
                        idx = db[db["單號"]==st.session_state.edit_id].index[0]
                        db.loc[idx, ["申請人", "代申請人", "專案名稱", "專案負責人", "專案編號", "總金額", "請款說明", "幣別", "影像Base64"]] = [app_val, proxy_val, pn, exe, pi, amt, packed_desc, currency, b_ims]
                        ...
                        nr = {"單號":tid, "日期":str(datetime.date.today()), "類型":sys_save_type, "申請人":app_val, "代申請人":proxy_val, "專案負責人":exe, "專案名稱":pn, "專案編號":pi, "請款說明":packed_desc, "總金額":amt, "幣別":currency, "狀態":"已儲存", "影像Base64":b_ims, "提交時間":"", "初審人":"", "初審時間":"", "複審人":"", "複審時間":"", "刪除人":"", "刪除時間":"", "刪除原因":"", "駁回原因":""}
        ...
        if st.session_state.last_id:
            ...
            if c4.button("✏️ 修改", disabled=not can_act): 
        ...
        if not is_admin: 
            my_db = my_db[my_db["申請人"] == curr_name]
        ...
        for i, r in p_df.iterrows():
            ...
            with c6:
                ...
                if b2.button("✅ 核准", key=f"ceo_ok_{i}", disabled=not can_sign):
                    ...
                    send_line_message(f"🔔 【報價單核准】\n單號：{r['單號']}\n專案：{r['專案名稱']}\n執行長已核准此報價單，請財務長進行最終複審！")
                    save_data(fresh_db)
                    st.rerun()
                ...
                if is_cfo_action:
                    with b3.popover("❌ 駁回"):
                        ...
                        if st.button("確認", key=f"cno_{i}"):
                            fresh_db = load_data()
                            idx = fresh_db[fresh_db["單號"]==r["單號"]].index[0]
                            fresh_db.loc[idx, ["狀態", "駁回原因", "複審人", "複審時間"]] = ["已駁回", reason, curr_name, get_taiwan_time()]
                            save_data(fresh_db)
                            st.rerun()
                else: 
...
# ================= 頁面 4: 表單狀態總覽及轉請款單 =================
    ...
    if not display_df.empty:
        st.info("💡 勾選報價單並「雙擊」下方輸入框填寫【本次請款金額】，確認無誤後點擊送出，即可一鍵建立新的請款單草稿！(請注意：請款金額不得超過尚未請款金額)")
        
        display_df.insert(0, "轉成請款單", False)
        display_df.insert(1, "本次請款金額(點擊輸入)", 0)
        display_df["負責執行長"] = display_df["專案負責人"]
        display_df["報價總金額"] = display_df.apply(lambda x: f"{str(x.get('幣別','TWD')).replace('nan','TWD')} ${clean_amount(x.get('總金額',0)):,.0f}", axis=1)
        ...
        target_cols = ["轉成請款單", "本次請款金額(點擊輸入)", "申請單號", "專案名稱", "負責執行長", "申請人", "報價總金額", "狀態", "請款狀態", "已請款金額", "尚未請款金額"]
        
        edited_df = st.data_editor(
            display_df[target_cols], 
            disabled=["申請單號", "專案名稱", "負責執行長", "申請人", "報價總金額", "狀態", "請款狀態", "已請款金額", "尚未請款金額"], 
            hide_index=True, 
            use_container_width=True,
            column_config={
        ...
        if st.button("🚀 確認將勾選項目轉成請款單"):
            ...
            for i, row in edited_df.iterrows():
                ...
                if bool(row.get("轉成請款單")) and pd.notna(conv_amt) and int(float(conv_amt)) > 0:
                    ...
                    nr = {
                        ...
                        "請款說明": f"自報價單 {orig_id} 轉換 (轉換金額: {real_conv_amt})", "總金額": real_conv_amt, 
            ...
            if converted_count > 0:
                save_data(fresh_db)
                st.success(f"✅ 成功轉換 {converted_count} 筆！請切換至「請款單系統」進行後續提交。")
                time.sleep(1.5)
                st.rerun()
            elif not has_error: 
    ...
    try:
        ...
        if not display_df.empty:
            ...
            edited_df = st.data_editor(
                ...
                use_container_width=True,
                column_config={
                    "匯款狀態": st.column_config.SelectboxColumn("匯款狀態", options=["尚未匯款", "已匯款"], required=True, width="medium"), 
                    "匯款日期": st.column_config.DateColumn("匯款日期", format="YYYY-MM-DD", width="medium", min_value=datetime.date(2020, 1, 1), max_value=datetime.date(2030, 12, 31))
                }
            )
...
if st.session_state.view_id:
    ...
    try:
        r = load_data()
        r = r[r["單號"]==st.session_state.view_id]
        if not r.empty:
