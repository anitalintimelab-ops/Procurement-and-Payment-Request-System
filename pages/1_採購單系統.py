        ...
        if st.session_state.edit_id:
            ...
            if not r.empty:
                ...
                dv.update({
                    ...
                    "pay": str(row.get("付款方式", "現金")), 
                    "vdr": str(row.get("請款廠商", "")), 
                    "acc": str(row.get("匯款帳戶", "")), 
                    "desc": str(row.get("請款說明", "")), 
                    "ab64": str(row.get("帳戶影像Base64", "")), 
                    "ib64": str(row.get("影像Base64", "")), 
                    "pay_cond": str(row.get("支付條件", "")), 
                    "pay_inst": str(row.get("支付期數", "")), 
                    "final_amt": clean_amount(row.get("最後採購金額", 0)), 
                    "billed_amt": clean_amount(row.get("已請款金額", 0)), 
                    "unbilled_amt": clean_amount(row.get("尚未請款金額", 0)), 
                    "bill_stat": str(row.get("請款狀態", ""))
                })
            ...
            for i, row in edited_df.iterrows():
                ...
                if bool(row.get("轉成請款單")) and pd.notna(conv_amt) and int(float(conv_amt)) > 0:
                    ...
                    nr = {
                        ...
                        "請款說明": f"自採購單 {orig_id} 轉換 (轉換金額: {real_conv_amt})", "總金額": real_conv_amt, 
