...
def render_html(row):
    ...
    h += f'<h3 style="margin:10px 0 0 0; letter-spacing:5px;">採購單</h3></div>'
...
if menu == "1. 填寫申請單":
    ...
    try:
        ...
        with st.form("form"):
            ...
            if dv["ib64"]:
                st.write("✅ 已有憑證")
                del_ims = st.checkbox("❌ 刪除所有舊憑證", key=f"di_{mode_suffix}")
            f_ims = st.file_uploader("上傳報價單/憑證 (支援圖片與PDF)", type=["png", "jpg", "jpeg", "pdf"], accept_multiple_files=True, key=f"fi_{mode_suffix}")
                ...
                if not (pn and pi and amt>0 and desc):
                    ...
                    if st.session_state.edit_id:
                        ...
                        nr = {"單號":tid, "日期":str(datetime.date.today()), "類型":sys_save_type, 
                              ...
                              "幣別":currency, "狀態":"已儲存", "影像Base64":b_ims, "提交時間":"",
                              "申請人信箱":curr_name, "初審人":"", "初審時間":"", "複審人":"", "複審時間":"", "刪除人":"", "刪除時間":"", "刪除原因":"", "駁回原因":"",
                              "支付條件": pay_cond, "支付期數": pay_inst, "請款狀態": bill_stat, "已請款金額": billed_amt, "尚未請款金額": amt, "最後採購金額": final_amt}
        ...
        if st.session_state.last_id:
            ...
            if c1.button("🚀 提交申請", disabled=not can_edit_or_submit):
...
# --- 頁面 2: 執行長簽核 ---
    ...
    try:
        sys_db = get_filtered_db()
        p_df = sys_db[(sys_db["狀態"].isin(["待簽核", "待初審"])) & ((sys_db["專案負責人"] == curr_name) | is_admin)]
        ...
        if h_df.empty: 
            ...
            for i, r in h_df.iterrows():
                ...
                with lc6:
                    ...
                    can_ceo_edit = (r["專案負責人"] == curr_name) and is_active and (r["狀態"] == "已核准")
                    if can_ceo_edit:
                        with lb3.popover("📝 更新"):
                            st.write("**📝 採購單後續修改**")
...
# --- 頁面 3: 財務長簽核 (功能保留) ---
    ...
    try:
        sys_db = get_filtered_db()
        st.subheader("⏳ 待財務長簽核")
        if is_admin or curr_name == CFO_NAME: p_df = sys_db[sys_db["狀態"] == "待複審"]
        else: p_df = sys_db[(sys_db["狀態"] == "待複審") & (sys_db["專案負責人"] == curr_name)]
        ...
        st.error(f"載入頁面時發生異常，錯誤訊息：{str(e)}")
        ...
        if not display_df.empty:
            ...
            edited_df = st.data_editor(
                ...
                column_config={
                    ...
                    "本次請款金額": st.column_config.NumberColumn("本次請款金額 (雙擊輸入)", min_value=0, step=1),
                }
            )
...
# --- 頁面 5: 系統設定 ---
    ...
    render_header()
    st.subheader("⚙️ 系統設定與資料還原")
    
    with st.expander("💾 表單資料庫備份與還原", expanded=True):
        if os.path.exists(D_FILE):
            with open(D_FILE, "rb") as f:
                st.download_button("⬇️ 下載表單備份檔", f, file_name=f"時研系統表單備份_{datetime.date.today()}.csv", mime="text/csv")
        st.markdown("---")
        uploaded_db = st.file_uploader("⬆️ 上傳表單 CSV 檔進行還原", type=["csv"], key="up_db")
