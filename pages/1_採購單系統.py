...
def render_html(row):
    ...
    sub_time_str = str(row.get("提交時間", "")) or get_taiwan_time()
    chu_info = f"{clean_name(row.get('初審人',''))} {row.get('初審時間','')}".strip()
    fu_info = f"{clean_name(row.get('複審人',''))} {row.get('複審時間','')}".strip()
    ...
    h += f'<div style="text-align:center;border-bottom:2px solid #000;padding-bottom:10px;">{lg_html}<h2>時研國際設計股份有限公司</h2><h3>採購單</h3></div>'
    h += '<table style="width:100%;border-collapse:collapse;font-size:14px;margin-top:10px;" border="1">'
    h += f'<tr><td bgcolor="#eee" width="20%">單號</td><td>{row["單號"]}</td><td bgcolor="#eee" width="20%">執行長</td><td>{clean_name(row["專案負責人"])}</td></tr>'
    ...
    h += f'<p style="font-size:12px;">提交: {sub_time_str} | 初審: {chu_info} | 複審: {fu_info}</p></div>'
    
    # 附件預覽
    v = ""
    if row.get('影像Base64'):
        for img in row['影像Base64'].split('|'):
            if is_pdf(img): v += f'<div style="page-break-before:always;"><embed src="data:application/pdf;base64,{img}" width="100%" height="800px" /></div>'
            else: v += f'<div style="page-break-before:always;"><img src="data:image/jpeg;base64,{img}" width="100%"></div>'
    return h + v
...
if menu == "1. 填寫申請單":
    ...
    if st.session_state.edit_id:
        ...
        if not r.empty:
            ...
            dv.update({"pn": row["專案名稱"], "exe": row["專案負責人"], "pi": row["專案編號"], "amt": row["總金額"], "desc": row["請款說明"]})
    ...
    with st.form("po_form"):
        ...
        amt = c2.number_input("預計採購金額", value=int(dv["amt"]), min_value=0)
        desc = st.text_area("採購說明", value=dv["desc"])
        f_ims = st.file_uploader("上傳憑證/報價單 (可多張)", accept_multiple_files=True)
        ...
        if st.form_submit_button("💾 儲存並產生單號"):
            if not (pn and pi and amt > 0):
                ...
                tid = st.session_state.edit_id or f"{today_str}-{len(db[db['單號'].str.startswith(today_str)])+1:02d}"
                
                new_row = {"單號":tid, "日期":str(datetime.date.today()), "類型":"採購單", "申請人":curr_name, "專案負責人":exe, "專案名稱":pn, "專案編號":pi, "總金額":amt, "請款說明":desc, "影像Base64":b_ims, "狀態":"已儲存", "提交時間":""}
                
                if st.session_state.edit_id: db.update(pd.DataFrame([new_row]).set_index(db[db['單號']==tid].index))
                else: db = pd.concat([db, pd.DataFrame([new_row])], ignore_index=True)
    ...
    if st.session_state.last_id:
        ...
        if c2.button("🖨️ 線上列印"):
            fresh_db = load_data()
            target = fresh_db[fresh_db["單號"]==st.session_state.last_id].iloc[0]
            js = f"var w=window.open();w.document.write('{clean_for_js(render_html(target))}');w.print();w.close();"
    ...
    st.dataframe(my_db[["單號", "專案名稱", "總金額", "狀態", "提交時間"]], use_container_width=True)

elif menu == "2. 專案執行長簽核":
    render_header()
    st.subheader("🔍 專案執行長簽核區")
    sys_db = get_filtered_db()
    p_df = sys_db[(sys_db["狀態"] == "待簽核") & (sys_db["專案負責人"] == curr_name)]
    
    if p_df.empty: st.info("目前沒有待簽核的採購單。")
    else:
        for i, r in p_df.iterrows():
            with st.expander(f"單號: {r['單號']} - {r['專案名稱']} (${r['總金額']:,})"):
                ...
                if c2.button("❌ 駁回", key=f"rej_{i}"):
                    ...
                    save_data(fresh_db)
                    st.rerun()
                if c3.button("🔍 預覽", key=f"view_{i}"):
                    st.session_state.view_id = r["單號"]
                    st.rerun()

elif menu == "4. 表單狀態總覽":
    render_header()
    st.subheader("📊 全公司採購單狀態總覽")
    sys_db = get_filtered_db()
    st.dataframe(sys_db[["單號", "專案名稱", "負責執行長", "總金額", "狀態"]], use_container_width=True)
